...
export function ClusterCard({ cluster, index, onAck, onUnack }: ClusterCardProps) {
  ...
  const handleAckSubmit = () => {
    onAck(ackNote.trim() || undefined);
    setShowAckModal(false);
    setAckNote('');
  };

  const openAckModal = (e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  return (
    <>
          ...
          <div className="flex-1 min-w-0">
            ...
            {cluster.acked && cluster.ackedAt && (
              <p className="text-xs text-success/60 font-mono mt-1">
                ...
                {cluster.ackedBy && ` · ${cluster.ackedBy}`}
              </p>
            )}
          </div>
      ...
      {/* Ack Modal */}
      {showAckModal && (
        <dialog className="modal modal-open" onClick={() => setShowAckModal(false)}>
          <div className="modal-box max-w-sm" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-bold text-lg mb-1">Acknowledge</h3>
            <p className="text-sm text-base-content/50 mb-4">
              {cluster.operator} · {cluster.alertCount} alerts
            </p>
            <input
              ref={inputRef}
              type="text"
              ...
              onKeyDown={(e) => { if (e.key === 'Enter') handleAckSubmit(); }}
            />
            <div className="modal-action">
              <button className="btn btn-ghost" onClick={() => setShowAckModal(false)}>
                Batal
              </button>
              <button className="btn btn-success" onClick={handleAckSubmit}>
                ✓ Acknowledge
              </button>
            </div>
          </div>
